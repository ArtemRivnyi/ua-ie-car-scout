...
describe('Live Scrapers — Classic Cars (1985–1995)', { timeout: 60000 }, () => {
  ...
  it('RST.ua: classic listings', async () => {
    const ads = await scrapeRst(query, yearFrom, yearTo);
    assert(Array.isArray(ads), 'RST should return array');
    if (ads.length === 0) { console.warn('[WARNING] RST: 0 classic results'); return; }
    assert.ok(ads[0].year >= 1985 && ads[0].year <= 1995, `Year out of range: ${ads[0].year}`);
  });

  it('OLX.ua: classic listings', async () => {
    const ads = await scrapeOlx(query, yearFrom, yearTo);
    assert(Array.isArray(ads), 'OLX should return array');
    if (ads.length === 0) { console.warn('[WARNING] OLX: 0 classic results'); return; }
    assert.ok(ads[0].year >= 1985 && ads[0].year <= 1995, `Year out of range: ${ads[0].year}`);
  });
