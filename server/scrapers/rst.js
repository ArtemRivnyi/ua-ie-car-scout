...
export async function scrapeRst(query, yearFrom, yearTo) {
  ...
  try {
    ...
    console.error('[rst] Puppeteer failed, falling back to HTTP:', err.message);
    try {
      ...
      return parseRst(content, query, rates, yearFrom, yearTo);
    } catch (fetchErr) {
      console.error('[rst] HTTP fallback failed:', fetchErr.message);
      return [];
    }
  } finally {
